...
3. **执行与验证**：按「验证流程」节预算执行，超预算即降级并记录（禁全量 e2e，见该节纪律）；收尾汇报：改动 diff + 规格验证结果 + 降级/豁免说明；涉及视觉表现的改动附加「视觉待用户确认」清单。
...
- **模型能力与职责边界**：当前路由（DeepSeek 默认）仅支持文本、无识图能力——AI 侧验证基线 = 代码与规格工作流（静态审查 / 可断言规格 / DOM 与计算样式取证），禁止截图路径与「肉眼确认」类结论；视觉表现确认由用户亲自处理（RunPreview），AI 不判定审美达标
...
> **职责边界**：AI 侧验证基线 = 代码与规格工作流（静态审查 + 可断言规格 + DOM 与计算样式取证，见「项目约定」）；视觉表现确认（审美/观感/像素细节）由用户亲自处理（RunPreview），AI 不输出「看起来正常/美观」类结论。
...
- **视觉职责边界**：涉及视觉表现（布局观感/动效手感/像素细节）的改动，收尾汇报必须显式列出「视觉待用户确认」项；AI 侧只交付规格证据（计算样式/尺寸/断点/溢出断言），不输出「看起来正常/美观」类结论；用户确认后的审美修正按返工流程处理
