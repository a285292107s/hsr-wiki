 ...
 * - 页面不可见（document.hidden）时暂停绘制，恢复后自动续播
...
/**
 * 挂载多层 Spine 场景（每次调用创建全新独立场景；断点重建 = teardown + 重新 mount）。
...
export function mountSpineScene(options: SpineSceneMountOptions): SpineSceneController {
  const { container, layers, viewport, lib, onReady } = options;
  let cancelled = false;
  let rafId = 0;
  ...
  if (!gl) return { teardown }; // WebGL 不可用 → 保持视图层渐变兑底背景
    ...
    .then(() => {
      ...
      try {
        ...
        const frame = (now: number): void => {
          ...
          if (document.hidden) return; // 页面不可见时暂停绘制（rAF 保持调度，恢复自动续播，省 GPU 带宽）
