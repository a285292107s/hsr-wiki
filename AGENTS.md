...
pnpm test:e2e
pnpm test:e2e:update   # 刷新像素基线（确认改动是预期后；强制 =all 模式，1.62 的默认 changed 模式更新已有基线会静默不写）
...
- 运行：`pnpm test:e2e`；基线刷新 `pnpm test:e2e:update`（=`all` 全覆盖模式，Playwright 1.62 默认 `--update-snapshots` 为 changed——更新已有基线时静默 pass 不落盘，必须显式 `=all`）；基线截图提交 git（`e2e/snapshots/`）；CI（ci.yml e2e job）在 push main 与 PR 时自动运行（软门禁：失败仅通知不拦 push）；mobile-chromium project（Pixel 7）仅跑 layout（溢出/结构/console 守卫），不跑像素基线
