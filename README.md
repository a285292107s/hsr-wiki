...
- **资源三级解析**：公共小图标随站本地（local-first）→ nanoka 主源（持续更新）→ jsDelivr 回退（旧档补全）；URL 构造统一经 `services/cdn/` 纯函数解析
...
├── spine-lab/               # 研究线（Spine Lab）：研究文档（spine-lab/docs）+ 抓取/审计脚本（spine-lab/tools）
...
- **图片资源**：公共小图标（element / pathicon / trace / 遗器通用部位）与 spine-player 运行时、货币战争 Hero 视频随站本地分发（`public/data/cn/assets/`、`public/vendor/spine/`）；其余非 Spine 图标以 **nanoka 主源（持续更新）+ jsDelivr 回退（旧档补全）** 双源解析（统一经 `src/services/cdn/` 纯函数；trace 因 nanoka 源为占位图而保持 jsDelivr 主源）；Spine `.skel`/`.atlas` 与未收录分类走 `https://static.nanoka.cc`。全部图片 URL 构造统一经 `src/services/cdn/` 纯函数解析（本地优先 + 双源回退 + 请求失败 CSS 占位降级）。
